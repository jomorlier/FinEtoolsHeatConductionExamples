...
function T4NAFEMS_T3_algo()
    ...
    vtkexportmesh("T4NAFEMS--T3.vtk", connasarray(regions[1]["femm"].integdata.fes),  [geom.values Temp.values/100], FinEtools.MeshExportModule.T3;
    scalars=[("Temperature", Temp.values)])
    vtkexportmesh("T4NAFEMS--T3--base.vtk", connasarray(regions[1]["femm"].integdata.fes), [geom.values 0.0*Temp.values/100], FinEtools.MeshExportModule.T3)
